...
const KPI = ({ label, value, theme, onClick }) => {
  const currentTheme = themeConfig[theme];
  return (
    <button onClick={onClick} className={`p-3 rounded-lg text-center ${currentTheme.userBg} border ${currentTheme.userBorder} transition-transform transform hover:scale-105`}>
...
const Dashboard = ({ projectState, theme, asciiRef, onKpiClick }) => {
    const currentTheme = themeConfig[theme];
    
    const kpiData = {
        ips: projectState.map(t => t.ip),
        ports: [...new Set(projectState.flatMap(t => t.ports))],
        services: [...new Set(projectState.flatMap(t => t.services))],
        vulns: [...new Set(projectState.flatMap(t => t.vulnerabilities))],
        cves: [...new Set(projectState.flatMap(t => t.cves))],
        creds: [...new Set(projectState.flatMap(t => t.credentials))],
    };
    ...
    return (
        <div ref={asciiRef} className={`${currentTheme.mainBg} rounded-lg p-6 border ${currentTheme.mainBorder} ${currentTheme.asciiBorderClass}`}>
            ...
            <div className="grid grid-cols-3 gap-2">
                <KPI label="IPs" value={kpiData.ips.length} theme={theme} onClick={() => onKpiClick('Target IPs', kpiData.ips)} />
                <KPI label="Ports" value={kpiData.ports.length} theme={theme} onClick={() => onKpiClick('Open Ports', kpiData.ports)} />
                <KPI label="Services" value={kpiData.services.length} theme={theme} onClick={() => onKpiClick('Services', kpiData.services)} />
                <KPI label="Vulns" value={kpiData.vulns.length} theme={theme} onClick={() => onKpiClick('Vulnerabilities', kpiData.vulns)} />
                <KPI label="CVEs" value={kpiData.cves.length} theme={theme} onClick={() => onKpiClick('CVEs', kpiData.cves)} />
                <KPI label="Creds" value={kpiData.creds.length} theme={theme} onClick={() => onKpiClick('Credentials', kpiData.creds)} />
            </div>
        </div>
    );
};
...
const App = () => {
  ...
  // State for interactive dashboard list view
  const [isListViewModalOpen, setIsListViewModalOpen] = useState(false);
  const [listViewData, setListViewData] = useState({ title: '', items: [] });
  ...
  const listViewModalRef = useRef(null);
  ...
  useAsciiBorder(listViewModalRef, theme === '1337' && isListViewModalOpen);
  ...
  const handleLoadChatFile = (event) => {
    ...
    reader.onload = (e) => {
      try {
        ...
        if (loadedSession.chat && Array.isArray(loadedSession.projectState)) {
            ...
            setMessages([]);
            setProjectState([]);
            setSaveStatus('Error: Invalid or outdated session file format.');
        }
      } catch (error) {
    ...
    const schema = {
        ...
        items: {
            ...
            properties: {
                ...
                ports: { type: "ARRAY", items: { type: "STRING" }, description: "Open ports found on this IP." },
                services: { type: "ARRAY", items: { type: "STRING" }, description: "Services running on this IP." },
                vulnerabilities: { type: "ARRAY", items: { type: "STRING" }, description: "Vulnerabilities found on this IP." },
                cves: { type: "ARRAY", items: { type: "STRING" }, description: "CVEs associated with this IP." },
                credentials: { type: "ARRAY", items: { type: "STRING" }, description: "Credentials found for this IP." },
            },
        }
    };

    const prompt = `Analyze the following text from a cybersecurity context. Extract any mentioned targets and their associated details (ports, services, vulnerabilities, CVEs, credentials). Structure the output as a JSON array where each object represents a target IP. If you find information like a port or vulnerability but no specific IP is mentioned, associate it with the most recently discussed IP or the first IP in the existing project state if available.

    Existing Project State (for context):
    ${JSON.stringify(projectState, null, 2)}
    ...
    try {
        ...
        if (jsonText) {
            ...
            setProjectState(prevState => {
                ...
                extractedData.forEach(newTargetData => {
                    if (!newTargetData.ip) return;

                    const existingTargetIndex = updatedState.findIndex(t => t.ip === newTargetData.ip);

                    if (existingTargetIndex > -1) {
                        // Merge with existing target
                        const existingTarget = updatedState[existingTargetIndex];
                        Object.keys(newTargetData).forEach(key => {
                            if (Array.isArray(existingTarget[key])) {
                                const combined = [...existingTarget[key], ...(newTargetData[key] || [])];
                                existingTarget[key] = [...new Set(combined)];
                            }
                        });
                    } else {
                        // Add new target
                        updatedState.push({
                            ip: newTargetData.ip,
                            domain: '',
                            ports: [...new Set(newTargetData.ports || [])],
                            services: [...new Set(newTargetData.services || [])],
                            vulnerabilities: [...new Set(newTargetData.vulnerabilities || [])],
                            cves: [...new Set(newTargetData.cves || [])],
                            credentials: [...new Set(newTargetData.credentials || [])],
                            notes: '',
                        });
                    }
                });
  ...
  const handleGenerateReport = async () => {
    ...
    try {
      ...
      2.  **Scope:** The scope of the simulated engagement (e.g., CTF), including target IPs and domains.
      3.  **Methodology:** A detailed, chronological account of the steps taken, tools used, and techniques applied.
      4.  **Findings:** A detailed list of all discovered services, open ports, vulnerabilities, CVEs, and credentials for each target.
  ...
  const handleGenerateAttackPlan = async () => {
    ...
    const intelSummary = projectState.map(t => `
- Target IP: ${t.ip} (${t.domain || 'No domain'})
  - Open Ports: ${t.ports.join(', ') || 'None'}
  - Services: ${t.services.join(', ') || 'None'}
  - Known Vulnerabilities: ${t.vulnerabilities.join(', ') || 'None'}
  - Known CVEs: ${t.cves.join(', ') || 'None'}
  - Credentials (if any): ${t.credentials.join(', ') || 'None'}
    `).join('');
...
1. Recon the target 192.168.1.10 with Nmap –-sV –-sC to discover services.
2. Enumerate SMB shares on 192.168.1.10 using enum4linux to locate potential exploitation vectors.`;
  ...
  const handleKpiClick = (title, items) => {
    setListViewData({ title, items });
  ...
  const MindMapModal = ({ projectState, theme, onClose }) => {
    ...
    useEffect(() => {
        ...
        const ipCol = projectState.map(t => ({ id: `ip-${t.ip}`, label: t.ip, type: 'ip', ref: React.createRef() }));
        if (ipCol.length > 0) columns.push(ipCol);

        const portCol = [...new Set(projectState.flatMap(t => t.ports))].map(port => ({ id: `port-${port}`, label: port, type: 'port', ref: React.createRef() }));
        if (portCol.length > 0) columns.push(portCol);
        
        const serviceCol = [...new Set(projectState.flatMap(t => t.services))].map(service => ({ id: `service-${service}`, label: service, type: 'service', ref: React.createRef() }));
        if (serviceCol.length > 0) columns.push(serviceCol);

        const vulnCol = [...new Set(projectState.flatMap(t => t.vulnerabilities))].map(vuln => ({ id: `vuln-${vuln}`, label: vuln, type: 'vuln', ref: React.createRef() }));
        if (vulnCol.length > 0) columns.push(vulnCol);

        const cveCol = [...new Set(projectState.flatMap(t => t.cves))].map(cve => ({ id: `cve-${cve}`, label: cve, type: 'cve', ref: React.createRef() }));
        if (cveCol.length > 0) columns.push(cveCol);

        const credCol = [...new Set(projectState.flatMap(t => t.credentials))].map(cred => ({ id: `cred-${cred}`, label: cred, type: 'cred', ref: React.createRef() }));
  ...
  const ProjectStateModal = () => {
    const currentTheme = themeConfig[theme];
    const [activeTargetIp, setActiveTargetIp] = useState(projectState[0]?.ip || null);
    const [newItem, setNewItem] = useState({ key: 'ports', value: '' });
    const [newTarget, setNewTarget] = useState({ ip: '', domain: '' });
    
    useEffect(() => {
        if (!activeTargetIp && projectState.length > 0) {
            setActiveTargetIp(projectState[0].ip);
        }
        if (projectState.length === 0) {
            setActiveTargetIp(null);
        }
    }, [projectState, activeTargetIp]);

    const handleUpdateTargetField = (ip, field, value) => {
        setProjectState(prevState => prevState.map(target => 
            target.ip === ip ? { ...target, [field]: value } : target
        ));
    };

    const handleAddNewTarget = (e) => {
        e.preventDefault();
        if (!newTarget.ip.trim() || projectState.some(t => t.ip === newTarget.ip)) {
            // Add some user feedback here later
            return;
        }
        const newTargetObject = {
            ip: newTarget.ip.trim(),
            domain: newTarget.domain.trim(),
            ports: [], services: [], vulnerabilities: [], cves: [], credentials: [], notes: ''
        };
        setProjectState(prevState => [...prevState, newTargetObject]);
        setNewTarget({ ip: '', domain: '' });
        setActiveTargetIp(newTargetObject.ip);
    };

    const handleDeleteTarget = (ipToDelete) => {
        setProjectState(prevState => prevState.filter(t => t.ip !== ipToDelete));
        if (activeTargetIp === ipToDelete) {
            setActiveTargetIp(projectState.length > 1 ? projectState.find(t => t.ip !== ipToDelete).ip : null);
        }
    };

    const handleAddItem = (e) => {
        e.preventDefault();
        if (!newItem.value.trim() || !activeTargetIp) return;

        setProjectState(prevState => prevState.map(target => {
            if (target.ip === activeTargetIp) {
                const updatedList = [...new Set([...target[newItem.key], newItem.value.trim()])];
                return { ...target, [newItem.key]: updatedList };
            }
            return target;
        }));
    ...
    const handleRemoveItem = (key, valueToRemove) => {
        setProjectState(prevState => prevState.map(target => {
            if (target.ip === activeTargetIp) {
                const updatedList = target[key].filter(item => item !== valueToRemove);
                return { ...target, [key]: updatedList };
            }
            return target;
        }));
    };

    const activeTarget = projectState.find(t => t.ip === activeTargetIp);

    return (
        <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50">
            <div ref={projectStateModalRef} className={`${currentTheme.mainBg} rounded-lg p-8 w-11/12 md:w-3/4 lg:w-2/3 border ${currentTheme.mainBorder} ${currentTheme.mainNeonGlow} ${currentTheme.asciiBorderClass} flex flex-col`}>
                <div className={`flex justify-between items-center border-b-2 ${currentTheme.mainBorder} pb-4 mb-4`}>
                ...
                <div className="flex-grow max-h-[60vh] overflow-y-auto pr-4">
                    <form onSubmit={handleAddNewTarget} className="flex space-x-2 mb-4">
                        <input type="text" value={newTarget.ip} onChange={e => setNewTarget({...newTarget, ip: e.target.value})} placeholder="New Target IP..." className={`w-1/2 bg-gray-800 ${currentTheme.userText} border ${currentTheme.userBorder} rounded-md p-2`} />
                        <input type="text" value={newTarget.domain} onChange={e => setNewTarget({...newTarget, domain: e.target.value})} placeholder="Domain (optional)..." className={`w-1/2 bg-gray-800 ${currentTheme.userText} border ${currentTheme.userBorder} rounded-md p-2`} />
                        <button type="submit" className={`py-2 px-4 rounded-md font-bold ${currentTheme.accentBg} ${currentTheme.accentHover} ${currentTheme.accentText}`}>ADD</button>
                    </form>

                    {projectState.length > 0 && (
                        <div className="mb-4">
                            <label className="block text-sm mb-1">ACTIVE TARGET</label>
                            <select value={activeTargetIp || ''} onChange={e => setActiveTargetIp(e.target.value)} className={`${currentTheme.dropdownBg} ${currentTheme.dropdownText} border ${currentTheme.dropdownBorder} rounded-md p-2 w-full`}>
                                {projectState.map(t => <option key={t.ip} value={t.ip}>{t.ip} {t.domain && `(${t.domain})`}</option>)}
                            </select>
                        </div>
                    )}
                    ...
                    {activeTarget ? (
                        <div>
                            <div className="flex justify-between items-center">
                                <h4 className="text-lg font-bold">Intel for {activeTarget.ip}</h4>
                                <button onClick={() => handleDeleteTarget(activeTarget.ip)} className={`${currentTheme.redText} hover:${currentTheme.redHover}`}>Delete Target</button>
                            </div>
                             <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-2">
                                {['ports', 'services', 'vulnerabilities', 'cves', 'credentials'].map(key => (
                                    <div key={key}>
                                        <h5 className="font-bold capitalize">{key}</h5>
                                        <ul className="h-24 overflow-y-auto bg-gray-800 p-2 rounded-md space-y-1">
                                            {activeTarget[key].map((item, index) => (
                                                <li key={index} className="flex justify-between items-center group text-sm">
                                                    <span className="break-all">{item}</span>
                                                    <button onClick={() => handleRemoveItem(key, item)} className="opacity-0 group-hover:opacity-100 text-red-500 hover:text-red-400">X</button>
                                                </li>
                                            ))}
                                        </ul>
                                    </div>
                            ...
                            <div className="mt-4">
                                ...
                                <textarea 
                                    ...
                                    onChange={e => handleUpdateTargetField(activeTarget.ip, 'notes', e.target.value)}
                                    className={`w-full h-24 bg-gray-800 ${currentTheme.userText} border ${currentTheme.userBorder} rounded-md p-2`}
                                    placeholder="Add notes for this target..."
                                />
                            </div>
                        </div>
                    ) : <p className="text-gray-500 italic text-center mt-8">No targets defined. Add a target IP to begin.</p>}
                </div>

                {activeTarget && (
                    <form onSubmit={handleAddItem} className={`mt-4 pt-4 border-t ${currentTheme.mainBorder}`}>
                        <h4 className="text-lg font-bold mb-2">ADD INTEL TO {activeTarget.ip}</h4>
                        <div className="flex space-x-2">
                            <select value={newItem.key} onChange={(e) => setNewItem({ ...newItem, key: e.target.value })} className={`flex-shrink-0 ${currentTheme.dropdownBg} ${currentTheme.dropdownText} border ${currentTheme.dropdownBorder} rounded-md p-2`}>
                            ...
                            <input type="text" value={newItem.value} onChange={(e) => setNewItem({ ...newItem, value: e.target.value })} className={`w-full bg-gray-800 ${currentTheme.userText} border ${currentTheme.userBorder} rounded-md p-2`} placeholder="Enter value..." />
                            <button type="submit" className={`py-2 px-4 rounded-md font-bold ${currentTheme.accentBg} ${currentTheme.accentHover} ${currentTheme.accentText}`}>ADD</button>
                        </div>
                    </form>
                )}
  ...
  const ListViewModal = () => {
    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50">
            <div ref={listViewModalRef} className={`${currentTheme.mainBg} rounded-lg p-8 w-11/12 md:w-1/2 lg:w-1/3 border ${currentTheme.mainBorder} ${currentTheme.mainNeonGlow} ${currentTheme.asciiBorderClass} flex flex-col`}>
                <div className={`flex justify-between items-center border-b-2 ${currentTheme.mainBorder} pb-4 mb-4`}>
                    <h3 className={`text-2xl font-bold ${currentTheme.mainText} ${currentTheme.mainNeonGlow}`}>{listViewData.title}</h3>
                    <button onClick={() => setIsListViewModalOpen(false)} className={`${currentTheme.mainText} hover:${currentTheme.mainText}`}>
                        <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg>
                    </button>
                </div>
                <div className="flex-grow max-h-[60vh] overflow-y-auto pr-2">
                    {listViewData.items.length > 0 ? (
                        <ul className="space-y-2">
                            {listViewData.items.map((item, index) => (
                                <li key={index} className={`flex justify-between items-center bg-gray-800 p-2 rounded-md group ${currentTheme.userText}`}>
                                    <span className="break-all mr-2">{item}</span>
                                    <button onClick={() => handleCopy(item, index)} className="text-gray-400 hover:text-white transition-opacity">
                                        {copiedItem === index ? (
                                            <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 text-green-400" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" /></svg>
                                        ) : (
                                            <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 16H6a2 2 0 01-2-2V6a2 2 0 012-2h8a2 2 0 012 2v2m-6 12h8a2 2 0 002-2v-8a2 2 0 00-2-2h-8a2 2 0 00-2 2v8a2 2 0 002 2z" /></svg>
                        ...
                        <p className="text-gray-500 italic text-center">No items to display.</p>
                    )}
                </div>
            </div>
        </div>
    ...
    <div className={`${currentTheme.mainBg} ${currentTheme.mainText} font-mono h-screen antialiased flex flex-col`}>
      ...
      <main className="flex-grow p-8 flex flex-col lg:flex-row lg:space-x-8 overflow-hidden">
        <div className="flex flex-col w-full lg:w-1/4 space-y-8 mb-8 lg:mb-0 h-full">
          ...
          <Dashboard projectState={projectState} theme={theme} asciiRef={dashboardRef} onKpiClick={handleKpiClick} />
        ...
        <div ref={mainWorkspaceRef} className={`flex flex-col w-full lg:w-3/4 ${currentTheme.mainBg} rounded-lg p-6 border ${currentTheme.mainBorder} h-full ${currentTheme.asciiBorderClass}`}>
          <div className={`relative flex justify-between items-center mb-4 pb-4 border-b ${currentTheme.mainBorder}`}>
            ...
            <h2 className={`text-xl font-bold absolute left-1/2 -translate-x-1/2 ${currentTheme.mainText} ${currentTheme.mainNeonGlow} ${glitchingTitle === 'navigator' ? 'glitch-active' : ''}`}>
              {theme === '1337' ? toLeetSpeak('CTF Navigator v2.12') : 'CTF Navigator v2.12'}
            </h2>
            <div className="flex space-x-2">
      ...
      {isListViewModalOpen && <ListViewModal />}
    </div>
  );
};
